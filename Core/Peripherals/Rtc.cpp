...
void Rtc::setTimeAndDate(const RtcBuffer& timeAndDate) const {
  RtcBuffer converted;
...
void Rtc::readTimeAndDate(RtcBuffer& buffer) {
  if (i2c_->memoryRead<0x00>(slaveAddress_, buffer.array) == hw::BusResult::Ok) {
