...
class Task : public Noncopyable {
 ...
 public:
   ...
   */
  constexpr Task(const char* name, StackDepth stackDepth, const Priority& priority,
                 void (*taskCode)(void*));
   ...
   */
  [[nodiscard]] inline auto getState() const -> TaskState;
 ...
 public:
   ...
   * @param miliseconds
   */
  inline static void delay(uint32_t miliseconds);
   ...
   * @param miliseconds
   */
  inline static void delayUntil(TickType_t* previousWakeTime, uint32_t miliseconds);
  ...
  /**
   * Allows the task to delete itself from scheduler. This method has to be static as the task's
   * function does not have access to non-static methods, since the object is not yet created.
   */
...
void Task::delay(const uint32_t miliseconds) { vTaskDelay(pdMS_TO_TICKS(miliseconds)); }
...
void Task::suspend() const { vTaskSuspend(taskHandle_); }

void Task::resume() const { vTaskResume(taskHandle_); }

void Task::suspendItself() { vTaskSuspend(nullptr); }

void Task::resumeItself() { vTaskResume(nullptr); }

auto Task::getState() const -> TaskState {
  return static_cast<TaskState>(eTaskGetState(taskHandle_));
}

void Task::resumeFromISR() const { portYIELD_FROM_ISR(xTaskResumeFromISR(taskHandle_)); }

void Task::deleteItself() { vTaskDelete(nullptr); }

void Task::deleteTask() const { vTaskDelete(taskHandle_); }

void Task::addToScheduler(void* params = nullptr) const {
  xTaskCreate(taskCode_, name_, stackDepth_, const_cast<void*>(params), static_cast<int>(priority_),
