...
Display::Display(hw::Lcd *lcd, const hw::Sensor::Buffer* sensorBuffer, const hw::Rtc::Buffer * timeAndDate)
...
void Display::init()
{
    displayTask_.addToScheduler(static_cast<void*>(&params_));
}
...
void Display::prepareDateData(std::array<char, 20> &lineBuffer, const hw::Rtc::Buffer * buf)
{
    ...
    std::sprintf(lineBuffer.data(), "    %s %'.02d/%'.02d/%'.02d", day_names[buf->weekday],
                 buf->day, buf->month, buf->year);
}
...
void Display::displayTaskFunction(void *args)
{
    ...
    while (true)
    {
        ...
        for(std::size_t i = 0; i < 4; ++i)
            params->lcd_->printLine(i, lineBuffers[i].data());
