...
public:
    ...
    inline BusResult transmit(uint16_t slaveAddress, uint8_t* data, uint16_t dataLength, uint32_t timeout = defaultTimeout_) const;
    template<uint16_t memoryAddress>
    inline BusResult memoryRead(uint16_t slaveAddress, uint8_t* data, uint16_t dataLength, uint32_t timeout = defaultTimeout_) const;
    template<uint16_t memoryAddress>
    inline BusResult memoryWrite(uint16_t slaveAddress, uint8_t* data, uint16_t dataLength, uint32_t timeout = defaultTimeout_) const;
private:
...
{
    return static_cast<BusResult>(HAL_I2C_Master_Transmit(handle_, slaveAddress, data, dataLength, timeout));
}
...
BusResult I2CBus::memoryRead(uint16_t slaveAddress, uint8_t* data, uint16_t dataLength, uint32_t timeout) const
{
    ...
    return static_cast<BusResult>(HAL_I2C_Mem_Read(handle_, slaveAddress, memoryAddress, addressSize, data, dataLength, timeout));
}
...
{
    constexpr auto addressSize = addressSizeOption(memoryAddress);
    return static_cast<BusResult>(HAL_I2C_Mem_Write(handle_, slaveAddress, memoryAddress, addressSize, data, dataLength, timeout));
}
